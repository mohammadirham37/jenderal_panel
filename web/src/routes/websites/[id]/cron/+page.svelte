...
import { language, translate } from '$lib/stores/language';
	...
	const schedulePresets = [
		...
		{ label: translate($language, 'wscronp.preset.every_5_minutes'), value: '*/5 * * * *' },
		{ label: translate($language, 'wscronp.preset.every_15_minutes'), value: '*/15 * * * *' },
		{ label: translate($language, 'wscronp.preset.hourly'), value: '0 * * * *' },
		{ label: translate($language, 'wscronp.preset.daily_midnight'), value: '0 0 * * *' },
		{ label: translate($language, 'wscronp.preset.weekly_sunday'), value: '0 0 * * 0' },
		{ label: translate($language, 'wscronp.preset.monthly'), value: '0 0 1 * *' }
	];
	...
	async function loadWebsite(requestedWebsiteID: string) {
		...
		if (!requestedWebsiteID) {
			websiteError = translate($language, 'wscronp.error.website_id_required');
		...
		try {
			...
			if (isCurrentRequest(requestedWebsiteID, generation)) {
				websiteError = err instanceof Error ? err.message : translate($language, 'wscronp.error.load_website');
			}
		} finally {
	...
	async function loadCronJobs(
		...
		try {
			...
			if (isCurrentRouteWebsite(requestedWebsiteID, generation)) {
				error = err instanceof Error ? err.message : translate($language, 'wscronp.error.load_cron_jobs');
			}
		} finally {
	...
	async function createCronJob() {
		...
		try {
			...
			if (!isCurrentRouteWebsite(requestedWebsiteID, generation)) return;
			toast.success(translate($language, 'wscronp.toast.created'));
			showCreateForm = false;
			createCommand = '';
			createSchedule = '';
			await loadCronJobs(scopedAPI, requestedWebsiteID, generation);
			...
			if (isCurrentRouteWebsite(requestedWebsiteID, generation)) {
				toast.error(err instanceof Error ? err.message : translate($language, 'wscronp.error.create'));
			}
		} finally {
	...
	async function toggleEnabled(job: CronJob) {
		...
		try {
			...
			if (isCurrentRouteWebsite(requestedWebsiteID, generation)) {
				toast.error(err instanceof Error ? err.message : translate($language, 'wscronp.error.toggle'));
			}
		}
	}
	...
	async function saveEdit(job: CronJob) {
		...
		try {
			...
			if (!isCurrentRouteWebsite(requestedWebsiteID, generation)) return;
			toast.success(translate($language, 'wscronp.toast.updated'));
			editingId = null;
			await loadCronJobs(scopedAPI, requestedWebsiteID, generation);
		} catch (err) {
			if (isCurrentRouteWebsite(requestedWebsiteID, generation)) {
				toast.error(err instanceof Error ? err.message : translate($language, 'wscronp.error.update'));
			}
		} finally {
	...
	async function deleteCronJob(id: string) {
		...
		try {
			...
			if (!isCurrentRouteWebsite(requestedWebsiteID, generation)) return;
			toast.success(translate($language, 'wscronp.toast.deleted'));
			await loadCronJobs(scopedAPI, requestedWebsiteID, generation);
		} catch (err) {
			if (isCurrentRouteWebsite(requestedWebsiteID, generation)) {
				toast.error(err instanceof Error ? err.message : translate($language, 'wscronp.error.delete'));
			}
		}
	}
...
<div class="space-y-6">
	{#if loadingWebsite}
		<div class="text-gray-400">{translate($language, 'wscronp.loading_website')}</div>
	...
	{:else if currentWebsite}
		<div class="flex items-center justify-between">
			<div>
				<a href="/websites" class="text-sm text-blue-400 hover:text-blue-300">{translate($language, 'wscronp.back_to_websites')}</a>
				<div class="flex flex-wrap items-center gap-3">
					<h2 class="text-2xl font-bold text-white">{translate($language, 'wscronp.title').replace('{domain}', currentWebsite.domain)}</h2>
					<span aria-label={translate($language, 'wscronp.website_status')} class="inline-block px-2.5 py-0.5 rounded bg-gray-700 text-xs font-medium text-gray-300">{currentWebsite.status}</span>
				</div>
			</div>
			<button onclick={() => (showCreateForm = !showCreateForm)} class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded transition-colors cursor-pointer">
				{showCreateForm ? translate($language, 'wscronp.cancel') : translate($language, 'wscronp.add_cron_job')}
			</button>
		</div>
		...
		{#if showCreateForm}
			<div class="bg-gray-800 rounded-lg border border-gray-700 p-5">
				<h3 class="text-lg font-semibold text-white mb-4">{translate($language, 'wscronp.new_cron_job')}</h3>
				<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
					<div>
						<label for="cron-command" class="block text-sm text-gray-400 mb-1">{translate($language, 'wscronp.command')}</label>
					...
					<div>
						<label for="cron-schedule" class="block text-sm text-gray-400 mb-1">{translate($language, 'wscronp.schedule')}</label>
				...
				<div class="mt-3 flex flex-wrap gap-2">
					<span class="text-xs text-gray-500 self-center">{translate($language, 'wscronp.presets')}</span>
				...
				<div class="mt-4">
					<button onclick={createCronJob} disabled={creating || !createCommand.trim() || !createSchedule.trim()} class="px-4 py-2 bg-green-600 hover:bg-green-700 disabled:opacity-50 text-white text-sm font-medium rounded transition-colors cursor-pointer">{creating ? translate($language, 'wscronp.adding') : translate($language, 'wscronp.add')}</button>
				</div>
			</div>
		{/if}

		{#if loading}
			<div class="text-gray-400">{translate($language, 'wscronp.loading_cron_jobs')}</div>
		{:else if error}
			<div class="p-4 bg-red-900/50 border border-red-700 rounded-lg text-red-300">{error}</div>
		{:else if cronJobs.length === 0}
			<div class="bg-gray-800 rounded-lg border border-gray-700 p-8 text-center"><p class="text-gray-400">{translate($language, 'wscronp.empty')}</p></div>
		{:else}
			<div class="bg-gray-800 rounded-lg border border-gray-700 overflow-hidden"><div class="overflow-x-auto"><table class="w-full">
				<thead><tr class="border-b border-gray-700">
					<th class="text-left px-4 py-3 text-xs text-gray-400 uppercase tracking-wider font-medium">{translate($language, 'wscronp.command')}</th>
					<th class="text-left px-4 py-3 text-xs text-gray-400 uppercase tracking-wider font-medium">{translate($language, 'wscronp.schedule')}</th>
					<th class="text-center px-4 py-3 text-xs text-gray-400 uppercase tracking-wider font-medium">{translate($language, 'wscronp.enabled')}</th>
					<th class="text-left px-4 py-3 text-xs text-gray-400 uppercase tracking-wider font-medium">{translate($language, 'wscronp.last_run')}</th>
					<th class="text-left px-4 py-3 text-xs text-gray-400 uppercase tracking-wider font-medium">{translate($language, 'wscronp.status')}</th>
					<th class="text-right px-4 py-3 text-xs text-gray-400 uppercase tracking-wider font-medium">{translate($language, 'wscronp.actions')}</th>
				</tr></thead>
				<tbody class="divide-y divide-gray-700">
					{#each cronJobs as job}
						<tr class="hover:bg-gray-750">
							...
							<td class="px-4 py-3 text-center"><button onclick={() => toggleEnabled(job)} class="relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 focus:outline-none {job.enabled ? 'bg-blue-600' : 'bg-gray-600'}" role="switch" aria-checked={job.enabled} aria-label={translate($language, 'wscronp.toggle_cron_job')}><span class="pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 {job.enabled ? 'translate-x-4' : 'translate-x-0'}"></span></button></td>
							...
							<td class="px-4 py-3 text-right"><div class="flex items-center justify-end gap-2">
								{#if editingId === job.id}
									<button onclick={() => saveEdit(job)} disabled={saving} class="px-2.5 py-1 bg-green-600 hover:bg-green-700 disabled:opacity-50 text-white text-xs rounded transition-colors cursor-pointer">{saving ? translate($language, 'wscronp.saving') : translate($language, 'wscronp.save')}</button>
									<button onclick={cancelEdit} class="px-2.5 py-1 bg-gray-600 hover:bg-gray-500 text-white text-xs rounded transition-colors cursor-pointer">{translate($language, 'wscronp.cancel')}</button>
